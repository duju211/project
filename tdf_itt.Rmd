...
preview: "https://github.com/duju211/tour_tt/blob/0a11c912b562f366506fcbe2607e3ba261ffb4ca/README_files/figure-markdown_strict/plot_total_time-1.png?raw=true"
...
Extract all urls for TdF editions from the last years
(up until year `r tar_read(start_year)`). Create the url of each edition
by adding the year to '`r tar_read(cycling_stats_url)`':
...
Scrape the stage overview for each url using the following helper function.
After a first introduction (next code chunk) we only alter the
url a little bit by applying the `polite::nod` function. Scraping is done
with `polite::scrape`. Keep only certain elements of the scraped html by looking
for CSS '`r tar_read(stages_urls_css)`'.
...
Summarise data per stage. Use this summary to arrange the different stages later
in the plot.
...
```{r plot_total_time, echo=FALSE, fig.height=7, fig.width=8}
